...
if(strolling) {
	if(stroll_timer <= 0){
		...
		if(_obstacle_ahead)
			move_direction *= -1;
		else if(player_distance <= 1.5*vision && move_direction != 0)
			move_direction = stroll_speed_multiplier * sign(obj_player.x-x);
	...
	if(is_grounded){
		if(player_xdistance <= PLAYER_SIZE*move_speed) move_direction = 0;
